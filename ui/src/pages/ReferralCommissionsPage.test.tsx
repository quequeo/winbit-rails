...
describe('ReferralCommissionsPage', () => {
  ...
  it('shows error when applying without investor', async () => {
    ...
    await waitFor(() => expect(screen.getByText('Seleccioná un inversor')).toBeInTheDocument());
  ...
  it('shows error when applying with invalid amount', async () => {
    ...
    await waitFor(() => expect(screen.getByText(/Ingresá un monto mayor a 0/)).toBeInTheDocument());
    expect(api.applyReferralCommission).not.toHaveBeenCalled();
  });
  ...
  it('applies commission successfully and shows success message', async () => {
    ...
    await waitFor(() => expect(api.applyReferralCommission).toHaveBeenCalledWith('1', { amount: 100 }));
    expect(screen.getByText('Comisión por referido aplicada correctamente')).toBeInTheDocument();
  });

  it('shows error when apply fails', async () => {
    const user = userEvent.setup();
    vi.mocked(api.applyReferralCommission).mockRejectedValue(new Error('Server error'));
  ...
  it('shows empty state when no history', async () => {
    ...
    expect(screen.getByText('No hay comisiones por referido registradas.')).toBeInTheDocument();
  });
  ...
  it('shows pagination when history has multiple pages', async () => {
    ...
    await waitFor(() => expect(api.getReferralCommissions).toHaveBeenCalledWith({ page: 2 }));
  });
});
