...
describe("DailyOperatingResultsPage", () => {
  ...
  it("disables apply when preview has zero impacted investors", async () => {
    ...
    expect(
      screen.getByText(/No hay inversores activos con capital para esa fecha/i),
    ).toBeInTheDocument();
  ...
  it("shows alert when preview API fails", async () => {
    const user = userEvent.setup();
    vi.mocked(api.previewDailyOperatingResult).mockRejectedValue(
  ...
  it("shows alert when apply fails", async () => {
    ...
    vi.mocked(api.createDailyOperatingResult).mockRejectedValueOnce(
  ...
  it("opens edit modal and runs edit preview", async () => {
    ...
    await user.click(
      screen.getByRole("button", { name: /Previsualizar cambio/i }),
    );

    await waitFor(() => {
      expect(api.editPreviewDailyOperatingResult).toHaveBeenCalledWith("abc", {
        percent: 2,
      });
    });
  ...
  it("applies edit after confirmation", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByText(/Confirmar edición de operativa/i),
      ).toBeInTheDocument(),
    );
    await user.click(
      screen.getByRole("button", { name: /Confirmar edición/i }),
    );

    await waitFor(() => {
      expect(api.updateDailyOperatingResult).toHaveBeenCalledWith(
        "abc",
