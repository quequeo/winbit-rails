...
describe("DepositOptionsPage", () => {
  ...
  describe("Listar opciones", () => {
    it("renders loading state initially", async () => {
      vi.mocked(api.getDepositOptions).mockImplementation(
        () => new Promise(() => {}),
      );
    ...
    it("renders options list after loading", async () => {
      ...
      expect(screen.getAllByText("Mercury Bank").length).toBeGreaterThan(0);
      expect(screen.getAllByText("USDT TRC20").length).toBeGreaterThan(0);
    });

    it("renders error message when fetch fails", async () => {
      vi.mocked(api.getDepositOptions).mockRejectedValue(
    ...
    it("renders empty state when no options", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/No hay opciones de depósito/),
        ).toBeInTheDocument();
      });
    });
    ...
    it('shows "Sin detalles" when option has no detail fields', async () => {
      ...
      vi.mocked(api.getDepositOptions).mockResolvedValue(
        optionsWithEmptyDetails,
      );
  ...
  describe("Crear opción", () => {
    ...
    it("creates new deposit option successfully", async () => {
      vi.mocked(api.getDepositOptions).mockResolvedValue(mockOptions);
      vi.mocked(api.createDepositOption).mockResolvedValue({
        data: { id: "3" },
      });
    ...
    it("shows error when create fails", async () => {
      vi.mocked(api.getDepositOptions).mockResolvedValue(mockOptions);
      vi.mocked(api.createDepositOption).mockRejectedValue(
        new Error('{"error":"Create failed"}'),
      );
      ...
      await user.type(
        screen.getByPlaceholderText("Ej: Banco Galicia - Pesos"),
        "Test",
      );
  ...
  describe("Toggle active", () => {
    it("toggles option active state", async () => {
      vi.mocked(api.getDepositOptions).mockResolvedValue(mockOptions);
      vi.mocked(api.toggleDepositOption).mockResolvedValue({
        data: { ...mockOptions.data[0], active: false },
      });
    ...
    it("shows error when toggle fails", async () => {
      vi.mocked(api.getDepositOptions).mockResolvedValue(mockOptions);
      vi.mocked(api.toggleDepositOption).mockRejectedValue(
  ...
  describe("Eliminar opción", () => {
    it("deletes option after confirmation", async () => {
      ...
      await waitFor(() =>
        expect(
          screen.getByText("Eliminar opción de depósito"),
        ).toBeInTheDocument(),
      );
    ...
    it("shows error when delete fails", async () => {
      vi.mocked(api.getDepositOptions).mockResolvedValue(mockOptions);
      vi.mocked(api.deleteDepositOption).mockRejectedValue(
      ...
      await user.click(
        screen.getAllByRole("button", { name: "Eliminar" }).pop()!,
      );
  ...
  describe("Editar opción", () => {
    ...
    it("shows error when update fails", async () => {
      vi.mocked(api.getDepositOptions).mockResolvedValue(mockOptions);
      vi.mocked(api.updateDepositOption).mockRejectedValue(
  ...
  describe("Form interactions", () => {
    it("cancels create form without submitting", async () => {
      ...
      expect(
        screen.queryByText("Nueva opción de depósito"),
