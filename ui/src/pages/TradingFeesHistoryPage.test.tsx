...
describe("TradingFeesHistoryPage", () => {
  ...
  it("loads history including voided fees and filters by status", async () => {
    ...
    await waitFor(() =>
      expect(api.getTradingFees).toHaveBeenCalledWith({
        include_voided: true,
        page: 1,
        per_page: 25,
      }),
    ...
    expect(
      screen.getAllByText(/Trading Fee por retiro/i).length,
    ).toBeGreaterThan(0);
    expect(screen.getAllByText(/Retiro: \$15,000\.00/i).length).toBeGreaterThan(
      0,
    );
