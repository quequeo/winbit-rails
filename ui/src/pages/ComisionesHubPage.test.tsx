...
describe("ComisionesHubPage", () => {
  beforeEach(() => {
    ...
    vi.mocked(api.getAdminInvestors).mockResolvedValue({ data: [] } as {
      data?: unknown[];
    });
    ...
    vi.mocked(api.getTradingFees).mockResolvedValue({ data: [] } as {
      data?: unknown[];
    });
  });
  ...
  it("renders tabs", async () => {
    ...
    expect(
      screen.getByRole("button", { name: "Comisiones por período" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "Comisiones por referido" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "Historial de Comisiones" }),
    ).toBeInTheDocument();
