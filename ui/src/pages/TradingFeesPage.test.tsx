...
describe('TradingFeesPage', () => {
  ...
  it('applies fee: calculate, confirm and apply', async () => {
    ...
    await waitFor(() => expect(screen.getAllByText('Investor Two').length).toBeGreaterThan(0))

    const applyButtons = screen.getAllByRole('button', { name: 'Aplicar' })
    await user.click(applyButtons[0])

    await waitFor(() => expect(screen.getByText('¿Aplicar comisión?')).toBeInTheDocument())
    ...
    expect(screen.getByText('Comisión aplicada exitosamente')).toBeInTheDocument()
  })
  ...
  it('shows error when calculate fails', async () => {
    const user = userEvent.setup()
    vi.mocked(api.getTradingFeesSummary).mockResolvedValue(mockRows)
    vi.mocked(api.calculateTradingFee).mockRejectedValue(new Error('Calculate failed'))
    ...
    await waitFor(() => expect(screen.getByText('Calculate failed')).toBeInTheDocument())
  })
  ...
  it('clears filter when Limpiar is clicked', async () => {
    ...
    await waitFor(() => expect(screen.getAllByText('Investor One').length).toBeGreaterThan(0))

    await user.type(screen.getByPlaceholderText('Buscar por nombre o email...'), 'one')
  ...
  it('opens detail modal when Detalle is clicked', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByText('Detalle de rentabilidad mensual')).toBeInTheDocument(),
    )
  })

  it('shows error when load fails', async () => {
    vi.mocked(api.getTradingFeesSummary).mockRejectedValue(new Error('Network error'))
  ...
  it('shows error when delete fails', async () => {
    const user = userEvent.setup()
    vi.mocked(api.getTradingFeesSummary).mockResolvedValue(mockRows)
    vi.mocked(api.deleteTradingFee).mockRejectedValueOnce(new Error('Delete failed'))
    ...
    const confirmBtn = screen.getAllByRole('button', { name: 'Eliminar' }).find((b) => (b.textContent || '').trim() === 'Eliminar')
  ...
  it('shows error when apply fails', async () => {
    ...
    vi.mocked(api.applyTradingFee).mockRejectedValueOnce(new Error('Apply failed'))
    ...
    await waitFor(() => expect(screen.getByText('Apply failed')).toBeInTheDocument(), { timeout: 3000 })
  })
  ...
  it('closes detail modal when Cerrar is clicked', async () => {
    ...
    await waitFor(() => expect(screen.getByText('Detalle de rentabilidad mensual')).toBeInTheDocument())

    await user.click(screen.getByRole('button', { name: 'Cerrar' }))
    await waitFor(() => expect(screen.queryByText('Detalle de rentabilidad mensual')).not.toBeInTheDocument())
  })
  ...
  it('closes flash when Cerrar is clicked', async () => {
    ...
    await waitFor(() => expect(screen.getByText('Comisión actualizada exitosamente')).toBeInTheDocument())
    const cerrarButtons = screen.getAllByRole('button', { name: 'Cerrar' })
    await user.click(cerrarButtons[cerrarButtons.length - 1])
    await waitFor(() => expect(screen.queryByText('Comisión actualizada exitosamente')).not.toBeInTheDocument())
  })
  ...
  it('navigates pagination with Siguiente and Anterior', async () => {
    ...
    await waitFor(() => expect(screen.getAllByText('Investor 0').length).toBeGreaterThan(0))

    expect(screen.getByText(/Filas por página/)).toBeInTheDocument()

    await user.click(screen.getByRole('button', { name: 'Siguiente' }))
    await waitFor(() => expect(screen.getAllByText('Investor 20').length).toBeGreaterThan(0))
  ...
  it('deletes from edit modal', async () => {
    ...
    await waitFor(() => expect(screen.getAllByText('Investor One').length).toBeGreaterThan(0))
    await user.click(screen.getAllByTitle('Editar')[0])
    await waitFor(() => expect(screen.getByText('Editar comisión aplicada')).toBeInTheDocument())

    const deleteBtnInEditModal = within(screen.getByText('Editar comisión aplicada').closest('div')!).getByRole('button', { name: 'Eliminar' })
    await user.click(deleteBtnInEditModal)

    await waitFor(() => expect(screen.getByText('Eliminar comisión aplicada')).toBeInTheDocument())
    const deleteConfirmModal = screen.getByText('Eliminar comisión aplicada').closest('.relative')!
    const confirmDelete = within(deleteConfirmModal).getByRole('button', { name: 'Eliminar' })
    await user.click(confirmDelete)

    await waitFor(() => expect(api.deleteTradingFee).toHaveBeenCalledWith('fee-1'))
  })
  ...
  it('covers frequency labels and no-profit branches in mobile and desktop', async () => {
    ...
    expect(screen.queryAllByRole('button', { name: /Detalle/i }).length).toBe(0)
  })
  ...
  it('shows errors when editing or deleting an applied fee without id', async () => {
    ...
    await user.click(screen.getAllByTitle('Editar')[0])
    await waitFor(() =>
      expect(screen.getByText('No se encontró el ID de la comisión aplicada')).toBeInTheDocument(),
    )
  ...
  it('validates edit percentage and handles update error', async () => {
    const user = userEvent.setup()
    vi.mocked(api.getTradingFeesSummary).mockResolvedValue(mockRows)
    vi.mocked(api.updateTradingFee).mockRejectedValueOnce(new Error('Update failed'))
  ...
  it('blurs number inputs on wheel in mobile and desktop lists', async () => {
    ...
    const enabledInputs = (screen.getAllByRole('spinbutton') as HTMLInputElement[]).filter((el) => !el.disabled)
  ...
  it('renders negative total style in monthly detail modal', async () => {
    ...
    await waitFor(() => {
      expect(screen.getAllByText('Investor Negative').length).toBeGreaterThan(0)
    })
  ...
  it('uses desktop delete action with applied fee id and shows zero total in detail modal', async () => {
    ...
    await waitFor(() => expect(api.deleteTradingFee).toHaveBeenCalledWith('fee-zero'))
