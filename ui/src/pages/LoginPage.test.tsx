...
describe("LoginPage", () => {
  ...
  it("renders login button with correct link", () => {
    ...
    expect(link).toHaveAttribute(
      "href",
      `${API_BASE_URL}/users/auth/google_oauth2`,
    );
  });
  ...
  it("renders disclaimer text", () => {
    render(<LoginPage />);
    expect(
      screen.getByText("Solo admins autorizados pueden acceder."),
    ).toBeInTheDocument();
  });
  ...
  it("shows unauthorized error message when error=unauthorized", () => {
    ...
    render(<LoginPage />);
    expect(
      screen.getByText("Tu cuenta de Google no está autorizada como admin."),
    ).toBeInTheDocument();
  });
  ...
  it("does not show error message when no error param", () => {
    ...
    render(<LoginPage />);
    expect(
      screen.queryByText(/Tu cuenta de Google no está autorizada/),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByText(/Falló el login con Google/),
    ).not.toBeInTheDocument();
  });
