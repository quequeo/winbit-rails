...
describe("DashboardPage", () => {
  ...
  it("renders dashboard data when loaded", async () => {
    ...
    expect(
      screen.getByRole("group", { name: "Rango de tiempo del gráfico" }),
    ).toBeInTheDocument();
  ...
  it("renders error message when fetch fails", async () => {
    vi.mocked(api.getAdminDashboard).mockRejectedValueOnce(
  ...
  it("calls API with days param when range selector changes", async () => {
    ...
    await waitFor(() =>
      expect(api.getAdminDashboard).toHaveBeenCalledWith({ days: 30 }),
    );
  });
  ...
  it("calls API with days 0 when Todo range is selected", async () => {
    ...
    await waitFor(() =>
      expect(api.getAdminDashboard).toHaveBeenCalledWith({ days: 0 }),
    );
  });
