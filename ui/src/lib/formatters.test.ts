...
describe("formatters", () => {
  ...
  describe("Argentine locale format consistency", () => {
    it("should use dot as decimal separator", () => {
      const currency = formatCurrencyAR(100.5);
      const number = formatNumberAR(100.5);
      const percent = formatPercentAR(100.5);

      // All should use dot for decimals (American format)
      expect(currency).toMatch(/[.]/);
      expect(number).toMatch(/[.]/);
      expect(percent).toMatch(/[.]/);
    });

    it("should use comma as thousands separator for large numbers", () => {
      const currency = formatCurrencyAR(10000.0);
      const number = formatNumberAR(10000.0);
