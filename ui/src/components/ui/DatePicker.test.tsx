...
describe('DatePicker', () => {
  ...
  it('opens calendar on click and selects a day', async () => {
    ...
    const dayButton = Array.from(gridButtons).find(
      (b) => /^\d+$/.test((b.textContent || '').trim()) || /^\d+$/.test((b as HTMLButtonElement).name || ''),
    );
  ...
  it('uses custom placeholder when provided', () => {
    const onChange = vi.fn();
    render(<DatePicker value="" onChange={onChange} placeholder="Seleccionar fecha" />);
  ...
  it('renders with empty value and allows selecting', async () => {
    ...
    const dayButton = Array.from(cells).find((b) => /^\d+$/.test((b as HTMLButtonElement).name || ''));
    if (dayButton) {
      await user.click(dayButton as HTMLElement);
      expect(onChange).toHaveBeenCalledWith(expect.stringMatching(/^\d{4}-\d{2}-\d{2}$/));
    }
